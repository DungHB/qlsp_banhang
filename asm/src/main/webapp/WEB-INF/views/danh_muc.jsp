                ...
                <form method="post">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="maDanhMuc" name="maDanhMuc"
                               placeholder="Nhập mã danh mục" required value="${danhMuc.maDanhMuc}">
                    </div>

                    <div class="form-group">
                        <label for="tenDanhMuc">Tên Danh Mục:</label>
                        <input type="text" class="form-control" id="tenDanhMuc" name="tenDanhMuc"
                               placeholder="Nhập tên danh mục" required value="${danhMuc.tenDanhMuc}">
                    </div>
                        ...
                        <div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" id="active" name="trangThai"
                                       value="Active" ${danhMuc.trangThai == 'Active' ? 'checked' : ''} checked>
                                <label class="form-check-label" for="active">Active</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" id="inactive" name="trangThai"
                                       value="Inactive" ${danhMuc.trangThai == 'Inactive' ? 'checked' : ''}>
                    ...
                    <div class="text-center">
                        <button formaction="/danh_muc/add" type="submit" class="btn btn-success">Thêm</button>
                        <button formaction="/danh_muc/upadate/${danhMuc.id}" type="submit" class="btn btn-success">Sửa
                        </button>
            ...
            <table class="table table-striped table-bordered">
                ...
                <c:forEach items="${listDM}" var="dmuc" varStatus="s">
                    <tr>
                        <td>${s.count}</td>
                        <td>${dmuc.id}</td>
                        <td>${dmuc.maDanhMuc}</td>
                        <td>${dmuc.tenDanhMuc}</td>
                        <td>${dmuc.trangThai}</td>
                        <td>${dmuc.ngayTao}</td>
                        <td>${dmuc.ngaySua}</td>
                        <td>
                            <a href="/danh_muc/delete/${dmuc.id}" class="btn btn-danger">Delete</a>
                            <a href="/danh_muc/detail/${dmuc.id}" class="btn btn-info">Detail</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
...
<%--<div class="container">--%>
<%--    <header class="row">--%>
<%--        <img src="images/banner_fptshop.webp" alt="" style="height: 200px;">--%>
<%--    </header>--%>
<%--    <nav class="navbar navbar-expand-lg bg-body-tertiary">--%>
<%--        <div class="container-fluid">--%>
<%--            <a class="navbar-brand" href="/home"><img src="images/Logo-FPT-Shop-Black.png" alt=""--%>
<%--                                                      style="height: 80px;"></a>--%>
<%--            <button class="navbar-toggler" type="button" data-bs-toggle="collapse"--%>
<%--                    data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"--%>
<%--                    aria-expanded="false" aria-label="Toggle navigation">--%>
...
<%--        <label>Họ tên: </label>--%>
<%--        <input class="form-control" type="text" value="${khDetail.hoTen}" name="hoTen"><br>--%>
<%--        <label>Địa chỉ: </label>--%>
<%--        <input class="form-control" type="text" value="${khDetail.diaChi}" name="diaChi"><br>--%>
<%--        <label>Số điện thoại: </label>--%>
<%--        <input class="form-control" type="text" value="${khDetail.sdt}" name="sdt"><br>--%>
<%--        <label>Trạng thái: </label>--%>
<%--        <input class="form-check-input" type="radio" name="trangThai"--%>
<%--               value="active" ${khDetail.trangThai == "active" ? "checked" : ""}--%>
<%--               checked>--%>
<%--        <label class="form-check-label">Active</label>--%>
<%--        <input class="form-check-input" type="radio" name="trangThai"--%>
<%--               value="inactive" ${khDetail.trangThai == "inactive" ? "checked" : ""}>--%>
<%--        <label class="form-check-label">Inactive</label>--%>
<%--        <br><br>--%>
<%--        <label>Ngày tạo: </label>--%>
<%--        <input class="form-control" type="date" name="ngayTao" ${khDetail.ngayTao}><br>--%>
<%--        <label>Ngày sửa: </label>--%>
<%--        <input class="form-control" type="date" name="ngaySua" ${khDetail.ngaySua}><br>--%>
...
<%--        </thead>--%>
<%--        <tbody>--%>
<%--        <c:forEach var="kh" items="${listKH}" varStatus="stt">--%>
<%--            <tr>--%>
<%--                <th>${stt.count}</th>--%>
<%--                <td>${kh.hoTen}</td>--%>
<%--                <td>${kh.diaChi}</td>--%>
...
<%--                <td>${kh.ngaySua}</td>--%>
<%--                <td>--%>
<%--                    <a href="/khach_hang/detail/${kh.id}" class="btn btn-info btn-sm">Detail</a>--%>
<%--                    <a href="/khach_hang/delete/${kh.id}" class="btn btn-danger btn-sm">Delete</a>--%>
<%--                </td>--%>
<%--            </tr>--%>
<%--        </c:forEach>--%>
<%--        </tbody>--%>
